export default (req, res, next) => {
    ...
    const allowedKeys = ['name', 'addresse', 'Latitude', 'Longitude', 'tel', 'website'];
    ...
    if (method === "POST") {
        ...
        if (typeof Latitude !== 'number' || Latitude.trim() === '') {
            return res.status(400).json({ message: 'Le champ "Latitude" doit être un nombre valide.' });
        }

        if (typeof Longitude !== 'number' || Longitude.trim() === '') {
        ...
        if ("Latitude" in req.body && (typeof Latitude !== 'number' || Latitude.trim() === '')) {
            return res.status(400).json({ message: 'Le champ "Latitude" doit être un nombre valide.' });
        }

        if ("Longitude" in req.body && (typeof Longitude !== 'number' || Longitude.trim() === '')) {
            return res.status(400).json({ message: 'Le champ "Longitude" doit être un nombre valide.' });
        }
